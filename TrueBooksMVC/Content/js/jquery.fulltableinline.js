				...
				$(table).find("thead th:not(.fulltable-edition-control):not(.fulltable-selection-control)").each(function (th_index, th) {
					...
					if (options.orderable) {
						...
						if (fieldData.orderable == null || fieldData.orderable == true) {
							var sortElement = $("<a/>").addClass("fulltable-sort").addClass("fulltable-sort-asc").addClass("fa").addClass("fa-sort-asc").html("");
							$(sortElement).click(function (event) {
								apply_order(true);
								order();
							});
							$(th).append(sortElement);
							var sortElement = $("<a/>").addClass("fulltable-sort").addClass("fulltable-sort-desc").html("");
							$(sortElement).click(function (event) {
								apply_order(true);
								order();
							});

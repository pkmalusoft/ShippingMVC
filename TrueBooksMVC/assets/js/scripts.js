    ...
    function qp_line_chart(chartID, maxHeight) {
        if ($(chartID).length) {
            ...
            clickedElement.on(triggeredEvent, function (e) {
                ...
                $.get("/DataFetch/GetChartData", { variant: range }, function (data) {
                    if (data != null) {
                        var xAxisLabels = data.Labels;
                        var dataSet2 = data.Orders;
                        var dataSet1 = data.Sales;
    ...
    function qp_doughnut_pie_chart(chartID, chartType, maxWidth) {
        ...
        if ($(chartID).length) {
            ...
            clickedElement.on(triggeredEvent, function (e) {
                ...
                $(this).siblings().removeClass('active');
                $(this).addClass('active');
                $.get("/DataFetch/GetDestData", { variant: range }, function (data) {
                    if (data != null) {
                        var xAxisLabels = data.Labels;
                        var dataSet = data.Orders;
                        load_chart(range, xAxisLabels, dataSet);
                    } else {
                    };
                });
                /* DEMO DATA - START */
            ...
            function load_chart(range, xAxisLabels, dataSet) {
        ...
        if ($(placeholder).length) {
            $(placeholder).each(function () {
                oTable = $(this).DataTable({
                    dom: 'Brt<"row" <"col-sm-1"l><"offset-md-3 col-md-4 col-sm-7 text-center"i><"offset-md-2 col-sm-4 col-md-2 text-sm-center text-md-right"p>>',
                    bPaginate: true,
                    bLengthChange: true,
                    bFilter: true,
                    bInfo: true,
...
function printDiv() {
    $('button.buttons-print').trigger('click');
}
function toasterMessage(command) {
    if (command != undefined || command != "" || command != null) {
        ...
        toastr[arr[0]](arr[1], arr[2]);
    }
}

function deleteFunction(id, controller) {
    swal({
        title: "Are you sure?",

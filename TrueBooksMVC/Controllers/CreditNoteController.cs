...
namespace TrueBooksMVC.Controllers
{
    ...
    public class CreditNoteController : Controller
    {
        ...
        public ActionResult Create()
        {
            ...
            var CreditNotes = db.CreditNotes.ToList().LastOrDefault();
            var CreditNo = 1;
            if (CreditNotes != null)
            {
                var CreditNumstr = CreditNotes.CreditNoteNo.Split('/');
                if (CreditNumstr.Count() > 1)
                {
                    CreditNo = Convert.ToInt32(CreditNumstr[1]) + 1;
                }
            }
            var CreditNumber = CreditNo.ToString("0000");

            var CreditnoteNum = "CN-" + datetime.ToString("ddMMyyyy") + "/" + CreditNumber;
            ViewBag.CreditNoteNum = CreditnoteNum;
            return View();
        ...
        public ActionResult Create(CreditNoteVM v)
        {
            ...
            invid = (from c in db.JInvoices where c.JobID == jobid select c.JobID).FirstOrDefault();
        ...
        {
            List<jobno> lst = new List<jobno>();
            var jobs = (from c in db.JobGenerations where c.ConsigneeID == id && c.JobCode.Contains(term) select c).ToList();
            if (jobs != null)
            {
                foreach (var item in jobs)
                {
        ...
        public JsonResult GetAmountByinvono(string invno, bool IsTrading)
        {
            ...
            int invid = (from c in db.JInvoices where c.JobID == jobid && c.InvoiceStatus == "1" && c.CancelledInvoice == false select c.InvoiceID).FirstOrDefault();

            decimal invamt = (from c in db.JInvoices where c.InvoiceID == invid && c.InvoiceStatus == "1" && c.CancelledInvoice == false select c.SalesHome).FirstOrDefault().Value;
            var recpay = (from r in db.CreditNotes where r.InvoiceID == jobid select r).ToList();
            if (recpay.Count > 0)
            {
                decimal? recamt = recpay.Sum(d => d.Amount);
                ob.recamt = recamt;
            }
            ob.invoiceamt = invamt;
